...
- `pnpm run check` passed 107 tests across 15 root test files, followed by the Cloudflare package typecheck and six focused tests. The Cloudflare tests cover authenticated health/model discovery, task-scoped NDJSON content and tool projection, provider failure and remote interruption, HTTPS-only credential transport, multi-message Pi event identity, Unicode-safe large image-bearing history split across bounded SQLite rows, VM path normalization, and the VM lifecycle surface.
